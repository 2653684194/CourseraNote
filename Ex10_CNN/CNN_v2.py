...
def col2img(X_col:np.ndarray, stride:int=1, padding:tuple=(0,0,0,0))->np.ndarray:
    '''
...
#         }
#     def get_optimizer_state(self):
#         return [layer.get_optimizer_state() for layer in self.Layers]
#     def set_config(self, config:dict):
...
#     def set_optimizer_state(self, optimizer_state:dict):
#         for layer, state in zip(self.Layers, optimizer_state):
...
#         if epsilon is not None:
#             self.epsilon = epsilon
#     def forward_prop(self,X:np.ndarray, training)->np.ndarray:
#         first_out = None
#         out = X
...
#                     # n1,n2, ..., nd == m1, m2, ..., md不能只判断n1*n2*...*nd == m1*m2*...*md
#                     self.W_proj = np.eye(input_size, output_size)
#                 else:
#                     self.W_proj = np.random.rand(input_size, output_size) * np.sqrt(2/(input_size+output_size))
#                 out += (first_out.reshape(1, input_size) @ self.W_proj).reshape(self.output_shape)
#         return out
#     def backward_prop(self,d_Z:np.ndarray)->np.ndaray:
#         connected_dZ = None
#         output_size = 1
#         for _ in (self.output_shape):
#             output_size *= _
#         for i in range(len(self.Layers)-1,-1,-1):
#             d_Z = layer.backward_prop(d_Z)
#             if (i == self.connected_layer):
#                 connected_dZ = (d_Z.reshape(1, output_size) @ self.W_proj.T).reshape(self.input_shape)
#                 d_W_proj  = d_Z.reshape(1, output_size) * np.ones(self.input_shape,self.output_shape)
#                 if self._Adam:
#                     self.V_W_proj = self.Adam_beta1 * self.V_W_proj + (1-self.Adam_beta1) * d_W_proj
#                     self.S_W_proj = self.Adam_beta2 * self.S_W_proj + (1-self.Adam_beta2) * np.power(d_W_proj,2)
#                     self.W_proj = self.W_proj - self.learning_rate * self.V_W_proj / (np.sqrt(self.S_W_proj) + self.epsilon)
#                 else:
#                     self.W_proj = self.W_proj - self.learning_rate * d_W_proj
