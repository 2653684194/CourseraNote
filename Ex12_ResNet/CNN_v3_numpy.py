...
class UpSampling(layer):# 这里采用双线性实现
    ...
    def get_interp_positions(self,H_src, W_src, H_targ, W_targ, align_corners=True):
        ...
        h_out = np.arange(H_targ)
        w_out = np.arange(W_targ)
        h_out_grid, w_out_grid = np.meshgrid(h_out, w_out, indexing='ij')  # (H_targ, W_targ)
    ...
    def forward_prop(self, A:np.ndarray)->np.ndarray:
        ...
        if len(A.shape)==4:
            ...
            if self.mode == 'bilinear':
                ...
                out = np.zeros((N, C, H_targ, W_targ))
                # 离某个点越近权重越大
    ...
    def backward_prop(self, d_Z:np.ndarray)->np.ndarray:
        ...
        if (self.w_tl==None or self.w_tr==None or self.w_bl==None or self.w_br==None):
            ...
            h_floor_flat = np.floor(self.h_in_grid).astype(int).reshape(-1,1) # (H_targ * W_targ, 1)
            w_floor_flat = np.floor(self.w_in_grid).astype(int).reshape(-1,1) # (H_targ * W_targ, 1)
            
            h_ceil_flat = np.minimum(h_floor_flat + 1, H_src - 1)
            w_ceil_flat = np.minimum(w_floor_flat + 1, W_src - 1)
            ...
            channel_offsets = np.arange(num_channels)[:, None] * num_pixels_in # (N*C, 1)
        ...
        d_A_flat = np.zeros(num_channels * num_pixels_in, dtype=d_Z.dtype) # (N*C*H_src*W_src, 1)
        ...
        np.add.at(d_A_flat, self.idx_tl, val_tl)
        np.add.at(d_A_flat, self.idx_tr, val_tr)
        np.add.at(d_A_flat, self.idx_bl, val_bl)
        np.add.at(d_A_flat, self.idx_br, val_br)
...
class ResBlock(TrainableLayer):
    ...
    # def safe_mul(self,A:np.ndarray,B:np.ndarray)->np.ndarray:
    #     max_size = 100000
    ...
    #         C=np.concatenate([np.concatenate([C_11,C_21],axis=1),np.concatenate([C_12,C_22],axis=1)],axis=0)
    #         return C
    #     else:
    #         return A @ B
                ...
                if len(self.src_shape) == 4 and len(self.target_shape) == 4:
                    if (self.src_shape == self.target_shape):
                        ...
                        if self.UpSampling_proj is None and \
                            ...
                            tmp = self.UpSampling_proj.forward_prop(tmp) # (N, C, H_targ, W_targ)
